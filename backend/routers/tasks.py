...
async def create_task(task: schemas.TaskCreate, db: DBDependency, current_username: CurrentUserDependency):
    user = database.get_user_by_username(db, current_username)
    if user is None:
        raise HTTPException(status_code=404, detail=f"User {current_username} does not exist")
    return database.create_task(db=db, task=task, user_id=user.id)
